...
# For mixture models
suppressPackageStartupMessages(library(mdir))
...
# For the Cranmer-von Mises test
suppressPackageStartupMessages(library(twosamples))
...
for (ii in seq(1, n_views)) {
  .x <- mdir::generateSimulationDataset(K = K[ii],
    N = N,
    P = P[ii],
...
We now believe that the chains have found whatever structure they are going to find by the 100th iteration, we run an ensemble of chains for an excessive 1,000 iterations.
...
data_2d <- generateGaussianDataset(cluster_means = c(-2, 2), 
  std_dev = c(1, 1), 
  N = N_2d, 
  P = P_2d, 
  pi = c(0.5, 0.5)
...
for(w in seq(1, W)) {
  .mcmc <- mdir::callMixtureModel(data_2d$data, D, thin = D, type = "G", K = 50)
  alloc_mat[w, ] <- .mcmc$allocations[2 , ]
}

cm <- mdir::createSimilarityMat(alloc_mat)
...
for(ii in seq(1, n_chains)) {
  .mcmc <- mdir::callMixtureModel(data_2d$data, R, thin, type = "G", K = 50)
  psms[[ii]] <- mdir::createSimilarityMat(.mcmc$allocations[-seq(1, burn / thin), ])
}
